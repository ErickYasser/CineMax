...
def processFile(update,bot,message,file,thread=None,jdb=None):
    ...
    if client:
        ...
        try:
            ...
            p = proxy.replace('socks5://', '')
            pr = S5Crypto.decrypt(f'{p}')  
            pro = f'socks5://{pr}'
            print(pro)
            ...
            if getUser['uploadtype'] == 'calendar' or getUser['uploadtype'] == 'draft':
                ...
                urls = asyncio.run(send_calendar(host,user,passw,nuevo,pro))
                loged = client.login()
                if loged:
                ...
                bot.sendMessage(message.chat.id,f'Enlace directo de calendario:\n\n{str(loco)}')
...
def onmessage(update,bot:ObigramClient):
    try:
        ...
        if username == tl_admin_user or user_info :  # validate user
            ...
            mensaje = "💢No tiene acceso.\nPor favor Contacta con @ErickYasser\n"
        ...
        if '/zips' in msgText:
            ...
            if getUser:
                try:
                   ...
                   jdb.save_data_user(username,getUser)
                   jdb.save()
                   msg = '🗜️Zips '+ sizeof_fmt(size*1024*1024)+' 📚'
                   bot.sendMessage(update.message.chat.id,msg)
                except:
        ...
        message = bot.sendMessage(update.message.chat.id,'⏳Iniciando...⌛')
        ...
        if '/start' in msgText:
            ...
            start_msg+= '👩🏻‍💻Para saber los comandos solamente es poer el comando /help \n'
            start_msg+= '🤖Para saber la vercion del bot /about \n'
            ...
            #    return
            bot.editMessageText(message,'⚠️Error no se pudo analizar⚠️')
    except Exception as ex:
           print(str(ex))
